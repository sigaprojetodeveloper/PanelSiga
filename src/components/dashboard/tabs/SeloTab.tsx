...
const SeloUserRow: React.FC<SeloUserRowProps> = ({ u, onSelectUser, userHook }) => {
  ...
  const updatedAtFormatted = formatUpdatedAt(u.verification_updated_at);
            ...
            <tbody>
              {userHook.loading ? (
                ...
                userHook.users.map((u: any) => (
                  <SeloUserRow key={u.id} u={u} onSelectUser={onSelectUser} userHook={userHook} />
                ))
              )}
            </tbody>
          </table>
